...
import { IBatchDetails, IBatchPayload, IPostBatchDetails } from "../model/IBatchDetails";
...
export class BatchService {
  ...
  static updateBatchService(id: string, bodyData: any) {
    ...
    return MongoService.collectionDetails("batch").then((obj) => {
      return obj.connection.findOneAndUpdate({ _id: new ObjectId(id) }, { $set: validBatchDetails }).finally(() => {
        obj.client.close();
      });
    });
  }
  static getSpecificBatchOfStudent(organisationId: string, branch: string, studentId: string) {
    return MongoService.collectionDetails("batch").then((obj) => {
      return obj.connection.findOne({ organisationId, branch, 'batchStudentList.studentId': studentId }).finally(() => {
        obj.client.close();
      });
    });
  }
  ...
  static getSpecificTeacherDetails(id: any) {
    return MongoService.collectionDetails("batch").then((obj) => {
      return obj.connection.find({ staffId: id }).toArray().finally(() => {
        obj.client.close();
      });
    });
  }
  ...
  static postBatchDetail(bodyContent: IBatchPayload, request: Request) {
    ...
    return MongoService.collectionDetails("user").then((userObj) => {
      return userObj.connection.findOne({ _id: new ObjectId(batchDetail.inCharge.id), userGroup: metaData.userGroup.staff }).then((staffDetails: IPostStaffDetails | any) => {
        if (staffDetails) {
          ...
          userObj.connection.findOneAndUpdate({ _id: new ObjectId(batchDetail.inCharge.id) }, {
            $set: {
    ...
    return MongoService.collectionDetails("batch").then((obj) => {
      return obj.connection
        .updateOne({ _id: new ObjectId(id) },
          { $addToSet: { studentList: { $each: studentSelectedToAdd.map((data: string) => new ObjectId(data)) } } })
        .then((data: any) => {
          for (const studentId of studentSelectedToAdd) {
            return MongoService.collectionDetails("user").then((obj) => {
              return obj.connection
                ...
                .updateOne({ _id: new ObjectId(studentId) }, { $set: { currentBatchId: new ObjectId(id) } })
            })
          }
        })
        .finally(() => {
  ...
  static removeStudentFromBatch(id: string, batchDetail: any) {
    ...
    return MongoService.collectionDetails("batch").then((obj) => {
      return obj.connection
        .updateOne({ _id: new ObjectId(id) }, { $pull: { studentList: new ObjectId(studentId) } })
        .then((data: any) => {
            return MongoService.collectionDetails("user").then((obj) => {
              return obj.connection
                // .updateOne({ _id: new ObjectId(studentId) }, { $push: { batch: { $each: [{ id: new ObjectId(id), name: batchName }] } } })
                .updateOne({ _id: new ObjectId(studentId) }, { $set: { currentBatchId: null } })
            })
        })
        .finally(() => {
          obj.client.close();
  ...
  static updateCompletedStatus(id: any, batchid: any, bodyData: any) {
    return MongoService.collectionDetails("user").then((obj) => {
      return obj.connection.findOne({ userGroup: metaData.userGroup.staff, _id: new ObjectId(id) }).then((data) => {
        if (data) {
          return MongoService.collectionDetails("batch").then((batchobj) => {
            return batchobj.connection.findOne({ _id: new ObjectId(batchid), "courseProgress": bodyData.id }).then((data1) => {
              if (data1) {
                return new Promise((resolve, reject) => {
                ...
                return batchobj.connection.findOneAndUpdate(
  ...
  static getBatchByQueryService(query: any) {
    if (query && query.course) {
      return MongoService.collectionDetails("batch").then((obj) => {
        return obj.connection.find({ "course.id": new ObjectId(query.course) }).toArray().finally(() => {
          obj.client.close()
        })
      })
    } else {
  ...
  static updateBatchModulesService(id: any, bodyData: any, updatedBy: any) {
    return MongoService.collectionDetails("user").then((obj) => {
      return obj.connection.findOne({ _id: new ObjectId(updatedBy), userGroup: metaData.userGroup.staff }).then(userData => {
        if (userData) {
          const assignedBatch = userData.assignedIn.map((doc: any) => doc.batchId.toString())
          if (assignedBatch.includes(id)) {
            return MongoService.collectionDetails("batch").then(async (batchObj) => {
              try {
                const batchData: any = await batchObj.connection.aggregate([
                ...
                if (batchModuleInfo && Object.keys(batchModuleInfo).length > 0) {
                ...
                const selectedModuleTopicId = totalCourseModuleInfo[bodyData.moduleId]
                if (!(selectedModuleTopicId && selectedModuleTopicId.includes(bodyData.topicId))) {
                  return new Promise((_, reject) => {
                    reject("Please check the module and topic id")
                  })
                }
                batchModuleInfo.topic.push(bodyData.topicId)
                const flag = selectedModuleTopicId.filter((id: any) => !batchModuleInfo.topic.includes(id)).length === 0
                if (flag) {
                  batchModuleInfo.module.push(bodyData.moduleId)
                }

                return batchObj.connection.updateOne({ _id: new ObjectId(id) }, { $set: { info: batchModuleInfo } }).finally(() => {
                  batchObj.client.close()
                })
